...
#WTP
deltaMethod_settings=list(expression=c(WTP_asc="asc/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_climate.6="mu_b_climate.6/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_climate.12="mu_b_climate.12/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_waterq.1="mu_b_waterq.1/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_waterq.2="mu_b_waterq.2/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_biodiversity.1="mu_b_biodiversity.1/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_biodiversity.2="mu_b_biodiversity.2/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_flood.1="mu_b_flood.1/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_flood.2="mu_b_flood.2/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_soiler.10="mu_b_soiler.10/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_soiler.20="mu_b_soiler.20/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)"),printPVal = TRUE)
apollo_deltaMethod(mxl2, deltaMethod_settings)

#add confidence Intervals
dm <- apollo_deltaMethod(mxl2, deltaMethod_settings)
dm <- as.data.frame(dm)
est_col <- grep("Value$", names(dm), ignore.case = TRUE, value = TRUE)[1]
se_col  <- grep("s.e.", names(dm), ignore.case = TRUE, value = TRUE)[1]
z <- qnorm(0.975)  # 95% CI
dm$ci_low  <- dm[[est_col]] - z * dm[[se_col]]
dm$ci_high <- dm[[est_col]] + z * dm[[se_col]]
dm

#Calculate means and medians of WTP distributions
# Extract estimates
b <- mxl2$estimate
...
# Draw cost coefficient
z_cost <- rnorm(N)

b_cost <- -exp(
  b["mu_log_b_cost"] +
    b["sigma_log_b_cost"] * z_cost
)

# Draw attribute coefficients
draw_wtp <- function(mu_name, sigma_name){
  ...
  # positive WTP because cost coefficient is negative
  wtp <- -b_attr / b_cost
  
  c(
    mean   = mean(wtp),
    median = median(wtp),
    p2.5   = quantile(wtp, 0.025),
    p95    = quantile(wtp, 0.95),
    p97.5  = quantile(wtp, 0.975)
  )
}

wtp_dist <- rbind(
  climate.6 = draw_wtp("mu_b_climate.6", "sigma_b_climate.6"),
  climate.12 = draw_wtp("mu_b_climate.12", "sigma_b_climate.12"),
  waterq.1 = draw_wtp("mu_b_waterq.1", "sigma_b_waterq.1"),
  waterq.2 = draw_wtp("mu_b_waterq.2", "sigma_b_waterq.2"),
  biodiversity.1 = draw_wtp("mu_b_biodiversity.1", "sigma_b_biodiversity.1"),
  biodiversity.2 = draw_wtp("mu_b_biodiversity.2", "sigma_b_biodiversity.2"),
  flood.1 = draw_wtp("mu_b_flood.1", "sigma_b_flood.1"),
  flood.2 = draw_wtp("mu_b_flood.2", "sigma_b_flood.2"),
  soiler.10 = draw_wtp("mu_b_soiler.10", "sigma_b_soiler.10"),
  soiler.20 = draw_wtp("mu_b_soiler.20", "sigma_b_soiler.20")
)
...
#####################################################################################################
# 3. Test for attribute non-attendance
#####################################################################################################

data_ana <- data[,c(1,30,32,34,36,38,40,42)]
colnames(data_ana) <- c("id","ana_none","ana_climate","ana_soiler","ana_waterq","ana_biodiversity",
                        "ana_flood","ana_costs")
database_covariates <- merge(database_covariates,data_ana,by.x = "ID",by.y = "id")

### Initialise
apollo_initialise()

apollo_control = list(
  modelName       = "MXL2_statedANA",
  modelDescr      = "MXL with stated ANA splits (attend vs ignore)",
  indivID         = "ID",
  mixing          = TRUE,
  nCores          = 2,
  seed            = 321,
...
# Starting values (use base estimates from model 2 as a sensible start for *_att,
# and something smaller (e.g. half) for *_ign. Same for sigmas.
apollo_beta = c(
  asc = -1.22,
  
  # Climate (attend vs ignore)
  mu_b_climate.6_att    = 0.19,
  sigma_b_climate.6_att = 0.80,
  mu_b_climate.6_ign    = 0.10,
  sigma_b_climate.6_ign = 0.80,
  mu_b_climate.12_att    = 0.58,
  sigma_b_climate.12_att = 0.88,
  mu_b_climate.12_ign    = 0.29,
  sigma_b_climate.12_ign = 0.88,
  
  # Water quality
  mu_b_waterq.1_att    = 0.96,
  sigma_b_waterq.1_att = 1.09,
  mu_b_waterq.1_ign    = 0.48,
  sigma_b_waterq.1_ign = 1.09,
  mu_b_waterq.2_att    = 1.08,
  sigma_b_waterq.2_att = 0.88,
  mu_b_waterq.2_ign    = 0.54,
  sigma_b_waterq.2_ign = 0.88,
  
  # Biodiversity
  mu_b_biodiversity.1_att    = 1.26,
  sigma_b_biodiversity.1_att = 0.72,
  mu_b_biodiversity.1_ign    = 0.63,
  sigma_b_biodiversity.1_ign = 0.72,
  mu_b_biodiversity.2_att    = 1.61,
  sigma_b_biodiversity.2_att = 1.06,
  mu_b_biodiversity.2_ign    = 0.80,
  sigma_b_biodiversity.2_ign = 1.06,
  
  # Flood
  mu_b_flood.1_att    = 0.70,
  sigma_b_flood.1_att = 0.90,
  mu_b_flood.1_ign    = 0.35,
  sigma_b_flood.1_ign = 0.90,
  mu_b_flood.2_att    = 0.59,
  sigma_b_flood.2_att = 0.07,
  mu_b_flood.2_ign    = 0.30,
  sigma_b_flood.2_ign = 0.07,
  
  # Soil erosion
  mu_b_soiler.10_att    = -0.08,
  sigma_b_soiler.10_att = 0.59,
  mu_b_soiler.10_ign    = -0.04,
  sigma_b_soiler.10_ign = 0.59,
  mu_b_soiler.20_att    = 0.23,
  sigma_b_soiler.20_att = 0.08,
  mu_b_soiler.20_ign    = 0.12,
  sigma_b_soiler.20_ign = 0.08,
  
  # Cost 
  mu_log_b_cost_att    = -5.21,
  sigma_log_b_cost_att =  0.96,
  mu_log_b_cost_ign    = -5.21,  #starting guess: same mean
  sigma_log_b_cost_ign =  0.96
)
...
apollo_draws = list(
  ...
  interNDraws    = 1000,
  interNormDraws = c(
    "draws_climate_inter.6","draws_climate_inter.12",
    "draws_waterq_inter.1","draws_waterq_inter.2",
    "draws_biodiversity_inter.1","draws_biodiversity_inter.2",
    "draws_flood_inter.1","draws_flood_inter.2",
    "draws_soiler_inter.10","draws_soiler_inter.20",
    "draws_cost_inter"
  )
)
...
apollo_randCoeff = function(apollo_beta, apollo_inputs){
  ...
  # Climate
  randcoeff[["b_climate.6_att"]]  = mu_b_climate.6_att  + sigma_b_climate.6_att  * draws_climate_inter.6
  randcoeff[["b_climate.6_ign"]]  = mu_b_climate.6_ign  + sigma_b_climate.6_ign  * draws_climate_inter.6
  randcoeff[["b_climate.12_att"]] = mu_b_climate.12_att + sigma_b_climate.12_att * draws_climate_inter.12
  randcoeff[["b_climate.12_ign"]] = mu_b_climate.12_ign + sigma_b_climate.12_ign * draws_climate_inter.12
  
  # Water quality
  randcoeff[["b_waterq.1_att"]] = mu_b_waterq.1_att + sigma_b_waterq.1_att * draws_waterq_inter.1
  randcoeff[["b_waterq.1_ign"]] = mu_b_waterq.1_ign + sigma_b_waterq.1_ign * draws_waterq_inter.1
  randcoeff[["b_waterq.2_att"]] = mu_b_waterq.2_att + sigma_b_waterq.2_att * draws_waterq_inter.2
  randcoeff[["b_waterq.2_ign"]] = mu_b_waterq.2_ign + sigma_b_waterq.2_ign * draws_waterq_inter.2
  
  # Biodiversity
  randcoeff[["b_biodiversity.1_att"]] = mu_b_biodiversity.1_att + sigma_b_biodiversity.1_att * draws_biodiversity_inter.1
  randcoeff[["b_biodiversity.1_ign"]] = mu_b_biodiversity.1_ign + sigma_b_biodiversity.1_ign * draws_biodiversity_inter.1
  randcoeff[["b_biodiversity.2_att"]] = mu_b_biodiversity.2_att + sigma_b_biodiversity.2_att * draws_biodiversity_inter.2
  randcoeff[["b_biodiversity.2_ign"]] = mu_b_biodiversity.2_ign + sigma_b_biodiversity.2_ign * draws_biodiversity_inter.2
  
  # Flood
  randcoeff[["b_flood.1_att"]] = mu_b_flood.1_att + sigma_b_flood.1_att * draws_flood_inter.1
  randcoeff[["b_flood.1_ign"]] = mu_b_flood.1_ign + sigma_b_flood.1_ign * draws_flood_inter.1
  randcoeff[["b_flood.2_att"]] = mu_b_flood.2_att + sigma_b_flood.2_att * draws_flood_inter.2
  randcoeff[["b_flood.2_ign"]] = mu_b_flood.2_ign + sigma_b_flood.2_ign * draws_flood_inter.2
  
  # Soil erosion
  randcoeff[["b_soiler.10_att"]] = mu_b_soiler.10_att + sigma_b_soiler.10_att * draws_soiler_inter.10
  randcoeff[["b_soiler.10_ign"]] = mu_b_soiler.10_ign + sigma_b_soiler.10_ign * draws_soiler_inter.10
  randcoeff[["b_soiler.20_att"]] = mu_b_soiler.20_att + sigma_b_soiler.20_att * draws_soiler_inter.20
  randcoeff[["b_soiler.20_ign"]] = mu_b_soiler.20_ign + sigma_b_soiler.20_ign * draws_soiler_inter.20
  
  # Cost (lognormal, negative), attend vs ignore
  randcoeff[["b_cost_att"]] = -exp(mu_log_b_cost_att + sigma_log_b_cost_att * draws_cost_inter)
  randcoeff[["b_cost_ign"]] = -exp(mu_log_b_cost_ign + sigma_log_b_cost_ign * draws_cost_inter)
...
apollo_probabilities = function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  # 0/1 masks (logical vectors)
  att_climate = (ana_climate == 0); ign_climate = (ana_climate == 1)
  att_waterq  = (ana_waterq  == 0); ign_waterq  = (ana_waterq  == 1)
  att_biodiv  = (ana_biodiversity == 0); ign_biodiv = (ana_biodiversity == 1)
  att_flood   = (ana_flood   == 0); ign_flood   = (ana_flood   == 1)
  att_soiler  = (ana_soiler  == 0); ign_soiler  = (ana_soiler  == 1)
  att_cost    = (ana_costs   == 0); ign_cost    = (ana_costs   == 1)
  
  V[["alt1"]] =
    # Climate split
    (b_climate.6_att  * (alt1.climate==6)  * att_climate) +
    (b_climate.6_ign  * (alt1.climate==6)  * ign_climate) +
    (b_climate.12_att * (alt1.climate==12) * att_climate) +
    (b_climate.12_ign * (alt1.climate==12) * ign_climate) +
    
    # Water quality split
    (b_waterq.1_att * (alt1.waterq==1) * att_waterq) +
    (b_waterq.1_ign * (alt1.waterq==1) * ign_waterq) +
    (b_waterq.2_att * (alt1.waterq==2) * att_waterq) +
    (b_waterq.2_ign * (alt1.waterq==2) * ign_waterq) +
    
    # Biodiversity split
    (b_biodiversity.1_att * (alt1.biodiversity==1) * att_biodiv) +
    (b_biodiversity.1_ign * (alt1.biodiversity==1) * ign_biodiv) +
    (b_biodiversity.2_att * (alt1.biodiversity==2) * att_biodiv) +
    (b_biodiversity.2_ign * (alt1.biodiversity==2) * ign_biodiv) +
    
    # Flood split
    (b_flood.1_att * (alt1.flood==1) * att_flood) +
    (b_flood.1_ign * (alt1.flood==1) * ign_flood) +
    (b_flood.2_att * (alt1.flood==2) * att_flood) +
    (b_flood.2_ign * (alt1.flood==2) * ign_flood) +
    
    # Soil erosion split
    (b_soiler.10_att * (alt1.soiler==10) * att_soiler) +
    (b_soiler.10_ign * (alt1.soiler==10) * ign_soiler) +
    (b_soiler.20_att * (alt1.soiler==20) * att_soiler) +
    (b_soiler.20_ign * (alt1.soiler==20) * ign_soiler) +
    
    # Cost split (lognormal, negative)
    (b_cost_att * alt1.cost * att_cost) +
    (b_cost_ign * alt1.cost * ign_cost)
  
  V[["alt2"]] =
    (b_climate.6_att  * (alt2.climate==6)  * att_climate) +
    (b_climate.6_ign  * (alt2.climate==6)  * ign_climate) +
    (b_climate.12_att * (alt2.climate==12) * att_climate) +
    (b_climate.12_ign * (alt2.climate==12) * ign_climate) +
    
    (b_waterq.1_att * (alt2.waterq==1) * att_waterq) +
    (b_waterq.1_ign * (alt2.waterq==1) * ign_waterq) +
    (b_waterq.2_att * (alt2.waterq==2) * att_waterq) +
    (b_waterq.2_ign * (alt2.waterq==2) * ign_waterq) +
    
    (b_biodiversity.1_att * (alt2.biodiversity==1) * att_biodiv) +
    (b_biodiversity.1_ign * (alt2.biodiversity==1) * ign_biodiv) +
    (b_biodiversity.2_att * (alt2.biodiversity==2) * att_biodiv) +
    (b_biodiversity.2_ign * (alt2.biodiversity==2) * ign_biodiv) +
    
    (b_flood.1_att * (alt2.flood==1) * att_flood) +
    (b_flood.1_ign * (alt2.flood==1) * ign_flood) +
    (b_flood.2_att * (alt2.flood==2) * att_flood) +
    (b_flood.2_ign * (alt2.flood==2) * ign_flood) +
    
    (b_soiler.10_att * (alt2.soiler==10) * att_soiler) +
    (b_soiler.10_ign * (alt2.soiler==10) * ign_soiler) +
    (b_soiler.20_att * (alt2.soiler==20) * att_soiler) +
    (b_soiler.20_ign * (alt2.soiler==20) * ign_soiler) +
    
    (b_cost_att * alt2.cost * att_cost) +
    (b_cost_ign * alt2.cost * ign_cost)
...
#model estimation
mxl2_statedANA <- apollo_estimate(
  apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs
)

apollo_modelOutput(mxl2_statedANA, list(printPVal = TRUE))
apollo_saveOutput(mxl2_statedANA, list(printPVal = TRUE))


# LR test against base model
LL_base <- mxl2$maximum
LL_ana  <- mxl2_statedANA$maximum
  
K_base <- length(mxl2$estimate)
K_ana  <- length(mxl2_statedANA$estimate)
df <- K_ana - K_base
  
LR <- 2 * (LL_ana - LL_base)
pval <- pchisq(LR, df=df, lower.tail=FALSE)
  
cat("\nLR test (stated ANA incl. cost vs base):\n",
      "LL_base =", LL_base, "\n",
      "LL_ana  =", LL_ana,  "\n",
      "LR      =", LR,      "\n",
      "df      =", df,      "\n",
      "p-value =", pval,    "\n\n")
#considering ANA improves the model

#cost comparison (whether the mean cost sensitivity differs between attenders 
#and non-attenders.

delta_cost = list(
  expression = c(
    mean_b_cost_att = "-exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    mean_b_cost_ign = "-exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    diff_mean_cost  = "(-exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)) - (-exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2))"
  ),
  printPVal = TRUE
)

apollo_deltaMethod(mxl2_statedANA, delta_cost)

#WTP for ANA-adjusted MXL model
deltaMethod_settings_ANA = list(
  expression = c(
    
    # ASC: use cost attenders / cost ignorers separately
    WTP_asc_cost_att = "asc/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_asc_cost_ign = "asc/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # CLIMATE = 6
    # --------------------------------------------------
    
    # Attribute attended, cost attended
    WTP_climate.6_att_cost_att = "mu_b_climate.6_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    # Attribute attended, cost ignored
    WTP_climate.6_att_cost_ign = "mu_b_climate.6_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    # Attribute ignored, cost attended
    WTP_climate.6_ign_cost_att = "mu_b_climate.6_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    # Attribute ignored, cost ignored
    WTP_climate.6_ign_cost_ign = "mu_b_climate.6_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # CLIMATE = 12
    # --------------------------------------------------
    
    WTP_climate.12_att_cost_att = "mu_b_climate.12_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_climate.12_att_cost_ign = "mu_b_climate.12_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_climate.12_ign_cost_att = "mu_b_climate.12_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_climate.12_ign_cost_ign = "mu_b_climate.12_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # WATER QUALITY = 1
    # --------------------------------------------------
    
    WTP_waterq.1_att_cost_att = "mu_b_waterq.1_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_waterq.1_att_cost_ign = "mu_b_waterq.1_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_waterq.1_ign_cost_att = "mu_b_waterq.1_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_waterq.1_ign_cost_ign = "mu_b_waterq.1_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # WATER QUALITY = 2
    # --------------------------------------------------
    
    WTP_waterq.2_att_cost_att = "mu_b_waterq.2_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_waterq.2_att_cost_ign = "mu_b_waterq.2_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_waterq.2_ign_cost_att = "mu_b_waterq.2_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_waterq.2_ign_cost_ign = "mu_b_waterq.2_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # BIODIVERSITY = 1
    # --------------------------------------------------
    
    WTP_biodiversity.1_att_cost_att = "mu_b_biodiversity.1_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_biodiversity.1_att_cost_ign = "mu_b_biodiversity.1_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_biodiversity.1_ign_cost_att = "mu_b_biodiversity.1_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_biodiversity.1_ign_cost_ign = "mu_b_biodiversity.1_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # BIODIVERSITY = 2
    # --------------------------------------------------
    
    WTP_biodiversity.2_att_cost_att = "mu_b_biodiversity.2_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_biodiversity.2_att_cost_ign = "mu_b_biodiversity.2_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_biodiversity.2_ign_cost_att = "mu_b_biodiversity.2_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_biodiversity.2_ign_cost_ign = "mu_b_biodiversity.2_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # FLOOD = 1
    # --------------------------------------------------
    
    WTP_flood.1_att_cost_att = "mu_b_flood.1_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_flood.1_att_cost_ign = "mu_b_flood.1_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_flood.1_ign_cost_att = "mu_b_flood.1_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_flood.1_ign_cost_ign = "mu_b_flood.1_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # FLOOD = 2
    # --------------------------------------------------
    
    WTP_flood.2_att_cost_att = "mu_b_flood.2_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_flood.2_att_cost_ign = "mu_b_flood.2_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_flood.2_ign_cost_att = "mu_b_flood.2_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_flood.2_ign_cost_ign = "mu_b_flood.2_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # SOIL EROSION = 10
    # --------------------------------------------------
    
    WTP_soiler.10_att_cost_att = "mu_b_soiler.10_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_soiler.10_att_cost_ign = "mu_b_soiler.10_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_soiler.10_ign_cost_att = "mu_b_soiler.10_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_soiler.10_ign_cost_ign = "mu_b_soiler.10_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",

    # --------------------------------------------------
    # SOIL EROSION = 20
    # --------------------------------------------------
    
    WTP_soiler.20_att_cost_att = "mu_b_soiler.20_att/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_soiler.20_att_cost_ign = "mu_b_soiler.20_att/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)",
    WTP_soiler.20_ign_cost_att = "mu_b_soiler.20_ign/exp(mu_log_b_cost_att + (sigma_log_b_cost_att^2)/2)",
    WTP_soiler.20_ign_cost_ign = "mu_b_soiler.20_ign/exp(mu_log_b_cost_ign + (sigma_log_b_cost_ign^2)/2)"
  ),
  
  printPVal = TRUE
)

apollo_deltaMethod(mxl2_statedANA,deltaMethod_settings_ANA)

dm_ANA <- apollo_deltaMethod(
  mxl2_statedANA,
  deltaMethod_settings_ANA
)

dm_ANA <- as.data.frame(dm_ANA)
est_col <- grep("Value$", names(dm_ANA),
                ignore.case = TRUE, value = TRUE)[1]
se_col <- grep("s.e.", names(dm_ANA),
               ignore.case = TRUE, value = TRUE)[1]
z <- qnorm(0.975)
dm_ANA$ci_low  <- dm_ANA[[est_col]] - z * dm_ANA[[se_col]]
dm_ANA$ci_high <- dm_ANA[[est_col]] + z * dm_ANA[[se_col]]

dm_ANA

#####################################################################################################
# 4. Test for concentration fatigue
#####################################################################################################

database_covariates$QES_order <- rep(1:18, length.out = nrow(database_covariates))
database_covariates$early <- as.integer(database_covariates$QES_order <= 6)
database_covariates$late  <- as.integer(database_covariates$QES_order >= 13)
#So, middle is the baseline

apollo_initialise()

apollo_control = list(
  modelName       = "MXL2_fatigue",
  modelDescr      = "MXL with test for fatigue effects",
  indivID         = "ID",
  mixing          = TRUE,
  nCores          = 2,
  seed            = 321,
...
apollo_beta=c(asc                      = -1.22, 
              ...
              mu_log_b_cost            = -5.21,
              sigma_log_b_cost         = 0.96,
              # Fatigue / learning scale parameters (mid block normalized to 1)
              delta_early              = 0,
              delta_late               = 0)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  #Scale factor by sequence block (mid block implicitly = 1)
  lambda <- exp(delta_early*early + delta_late*late)
  
  V = list()
  
  V[["alt1"]] = lambda * (
    b_climate.6*(alt1.climate==6) + b_climate.12*(alt1.climate==12) + b_waterq.1*(alt1.waterq==1) + 
      b_waterq.2*(alt1.waterq==2) + b_biodiversity.1*(alt1.biodiversity==1) + b_biodiversity.2*(alt1.biodiversity==2) +
      b_flood.1*(alt1.flood==1) + b_flood.2*(alt1.flood==2) + b_soiler.10*(alt1.soiler==10) + b_soiler.20*(alt1.soiler==20) + 
      b_cost*alt1.cost
  )
  ...
  V[["alt2"]] = lambda * (
    b_climate.6*(alt2.climate==6) + b_climate.12*(alt2.climate==12) + b_waterq.1*(alt2.waterq==1) + 
      b_waterq.2*(alt2.waterq==2) + b_biodiversity.1*(alt2.biodiversity==1) + b_biodiversity.2*(alt2.biodiversity==2) +
      b_flood.1*(alt2.flood==1) + b_flood.2*(alt2.flood==2) + b_soiler.10*(alt2.soiler==10) + b_soiler.20*(alt2.soiler==20) + 
      b_cost*alt2.cost
  )
...
mxl2_fatigue <- apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

apollo_modelOutput(mxl2_fatigue, list(printPVal = TRUE))
apollo_saveOutput(mxl2_fatigue, list(printPVal = TRUE))


#####################################################################################################
# 5. Simple MXL model with income- and education-interaction
#####################################################################################################
...
### Set core controls
apollo_control = list(
  modelName       = "MXL_interact_incedu",
  modelDescr      = "Simple MXL model with income- and education-interactions with the climate protrection attributes",
  indivID         = "ID",
  mixing          = TRUE,
  nCores          = 2,      
  seed            = 321,
...
# Mean-center income and education for interaction terms
database$income_c <- database$income_con - mean(database$income_con, na.rm = TRUE)
database$education_c <- database$education - mean(database$education, na.rm = TRUE)
...
apollo_beta=c(asc                      = -1.22, 
              ...
              sigma_log_b_cost         = 0.96,
              b_climate6_income        = 0,
              b_climate12_income       = 0,
              b_climate6_education     = 0,
              b_climate12_education    = 0)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  ### List of utilities
  V = list()
  V[["alt1"]] = b_climate.6*(alt1.climate==6) + b_climate.12*(alt1.climate==12) +
    # NEW: income x climate interactions
    b_climate6_income*income_c*(alt1.climate==6) + b_climate12_income*income_c*(alt1.climate==12) +
    # NEW: education x climate interactions
    b_climate6_education*education_c*(alt1.climate==6) + b_climate12_education*education_c*(alt1.climate==12) + 
    b_waterq.1*(alt1.waterq==1) + 
    b_waterq.2*(alt1.waterq==2) + b_biodiversity.1*(alt1.biodiversity==1) + b_biodiversity.2*(alt1.biodiversity==2) +
    b_flood.1*(alt1.flood==1) + b_flood.2*(alt1.flood==2) + b_soiler.10*(alt1.soiler==10) + b_soiler.20*(alt1.soiler==20) + 
    b_cost*alt1.cost
  V[["alt2"]] = b_climate.6*(alt2.climate==6) + b_climate.12*(alt2.climate==12) + 
    # NEW: income x climate interactions
    b_climate6_income*income_c*(alt2.climate==6) + b_climate12_income*income_c*(alt2.climate==12) +
    # NEW: education x climate interactions
    b_climate6_education*education_c*(alt2.climate==6) + b_climate12_education*education_c*(alt2.climate==12) + 
    b_waterq.1*(alt2.waterq==1) + 
    b_waterq.2*(alt2.waterq==2) + b_biodiversity.1*(alt2.biodiversity==1) + b_biodiversity.2*(alt2.biodiversity==2) +
    b_flood.1*(alt2.flood==1) + b_flood.2*(alt2.flood==2) + b_soiler.10*(alt2.soiler==10) + b_soiler.20*(alt2.soiler==20) + 
    b_cost*alt2.cost
  V[["alt3"]] = asc
...
mxl_interact_incedu <- apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(mxl_interact_incedu, list(printPVal = TRUE))
apollo_saveOutput(mxl_interact_incedu, list(printPVal = TRUE))
...
deltaMethod_settings=list(expression=c(WTP_asc="asc/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       ...
                                       WTP_soiler.10="mu_b_soiler.10/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)",
                                       WTP_soiler.20="mu_b_soiler.20/exp(mu_log_b_cost+(sigma_log_b_cost^2)/2)"),printPVal = TRUE)
apollo_deltaMethod(mxl_interact_incedu, deltaMethod_settings)

#add confidence Intervals
dm <- apollo_deltaMethod(mxl_interact_incedu, deltaMethod_settings)
dm <- as.data.frame(dm)
est_col <- grep("Value$", names(dm), ignore.case = TRUE, value = TRUE)[1]
se_col  <- grep("s.e.", names(dm), ignore.case = TRUE, value = TRUE)[1]
z <- qnorm(0.975)  # 95% CI
